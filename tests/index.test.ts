...
`,
    ...
    await expectGeneratedNotice(path.join(cwd, '.github/workflows/ci.yml'))

    const lockfile = await readYaml(path.join(cwd, '.github/workflow.lock.yml'))
    await expectGeneratedNotice(path.join(cwd, '.github/workflow.lock.yml'))
...
async function expectGeneratedNotice(file: string): Promise<void> {
